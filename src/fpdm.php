...
//else this is the pdf_source then, fdf content is loaded using Load() function
        ...
        function load_file($type, $content = NULL)
        {
            ...
            switch ($type) {
                case "PDF" :
                    if ($content) $this->Error("load_file do not accept PDF content, only FDF content sorry");
                    else $this->pdf_entries = $this->getEntries($this->pdf_source, 'PDF');
                    break;
                case "FDF" :
                    if (!is_null($content)) {
                        if (is_array($content)) {
                            ...
                            $this->fdf_parse_needed = true;
                        } else $this->Error('Invalid content type for this FDF file!');
                    } else {
        ...
        function Merge($flatten = false)
        {
            ...
            if ($this->support == "native") {
                ...
                if ($this->verbose && ($count_fields == 0)) $this->dumpContent("The FDF content has either no field data or parsing may failed", "FDF parser: ");
...
//							$this->set_field_value("tooltip",$name,$value);
                    ...
                    $this->fix_xref_start();
                } else $this->Error("PDF file is empty!");
        ...
        function Output($dest = '', $name = '')
        {
            ...
            if ($this->support == "pdftk") {
                ...
                if ($this->fdf_source) { //FDF file provided
                    ...
                    if (!$ret["success"]) $this->Error("Output failed as something goes wrong (Pdf was $pdf_url) <br> during internal FDF generation of file $fdf_file, <br>Reason is given by {$ret['return']}");
                }
            ...
            switch ($dest) {
                ...
                case 'F':
                    ...
                    $f = fopen($name, 'wb');
                    if (!$f) $this->Error('Unable to create output file: ' . $name . ' (currently opened under Acrobat Reader?)');
        ...
        {
            if ($this->isUTF8) $str = "\xFE\xFF" . iconv('UTF-8', 'UTF-16BE', $str);
...
//				echo $this->value_entries["$name"]["values"]["$type"];
                    ...
                    if (isset($this->value_entries[$name]["values"]["current"])) $offset_shift = $this->_set_field_value2($this->value_entries[$name]["values"]["current"], $value, false);
                    else $offset_shift = $this->_set_field_value2($this->value_entries[$name]["infos"]["name_line"], $value, true);
                }
//				}else
//					$this->Error("set_field_value failed as invalid valuetype $type for object $object_id");
            ...
            } else $this->Error("field $name not found");
        }
        ...
        function set_field_tooltip($name, $value)
        {
            ...
            } else $this->Error("set_field_tooltip failed as the field $name does not exist");
            return $offset_shift;
        }
        ...
        function getContent($filename, $filetype)
        {
            ...
            if (!$content) $this->Error(sprintf('Cannot open ' . $filetype . ' file %s !', $filename));

            if ($filetype == 'PDF') {
                $start                     = substr($content, 0, 2048);
                if (strpos($start, '/ObjStm') !== false) $this->Error('Object streams are not supported');
                if (strpos($start, '/Linearized') !== false) $this->Error('Fast Web View mode is not supported');
                $end                       = substr($content, -512);
                if (strpos($end, '/Prev') !== false) $this->Error('Incremental updates are not supported');
        ...
        function fix_xref_start()
        {
            ...
            if ($this->check_mode) { //Compared calculated value with position value read direct from file
                if ($xref_start_value_calculated != $xref_start_value_safe) {
                    ...
                    if ($this->halt_mode) $this->Error("Halt on error mode enabled, aborting. Use \$pdf->set_modes('halt',false); to disable this mode and go further fixing corrupted pdf.");
                } else {
        ...
        function decodeValue($encoding, $value)
        {
            ...
            if ($encoding == "hex") $value = $this->pdf_decode_field_value($value);
            return $value;
        }
        ...
        function extract_pdf_definition_value($name, $line, &$match)
        {
            ...
            } else $this->Error("extract_pdf_definition_value() does not support definition '$name'");
...
//									if(!array_key_exists("default",$values)) $this->Error("Cannot find default value (/DV) for field $name");
                                ...
                                if (array_key_exists("length", $stream)) { //length is mandatory
                                    ...
                                    if (preg_match("/^\/(V|DV|TU)\s+([\<\(])/", $CurLine, $def)) {
                                        ...
                                        if ($verbose_parsing) $this->dumpContent("$type $subtype (obj id=$obj) has $encoding $valuetype value [$value] at line $Counter");
                            ...
                            if ($refs_count > 0) {
                                ...
                                if ($trailer_table == 1) { //should be <<
                                    if (trim($CurLine) != '') { //HACK: PDFCreator Version 1.0.0 has an extra CR after trailer
                                        if (!preg_match("/<</", $CurLine, $match)) $this->Error("trailer_table corrupted?; missing start delimiter << ");
                                        $trailer_table++;
                                    }
                                    ...
                                    if (preg_match("/^\/ID\s*\[\s*<([\da-fA-F]+)/", $CurLine, $match)) {
                                        ...
                                        if (preg_match("/\>\s?\</", $CurLine, $match)) $id_single_line_def = true;
                                    }
                                    ...
                                    if ($id_def) {//we are inside the ID definition
                                        if ($id_single_line_def || $id_multi_line_def) {
                                            ...
                                            if (preg_match("/([\da-fA-F]+)>.*$/", $CurLine, $match)) {
                                                ...
                                                $id_def           = false;
                                            } else $this->Error("trailer_table corrupted?; ID chunk two can not be decoded ");
                                        } else $id_multi_line_def = true;
                                    }
                                    ...
                                    if (preg_match("/>>/", $CurLine, $match)) $trailer_table = -1; //negative value: expects startxref to follow
                                } else {

                                    switch ($trailer_table) {
                                        case -1://startxref
                                            if (!preg_match("/^startxref/", $CurLine, $match)) $this->Error("startxref tag expected, read $CurLine");
                                            break;
                                        case -2://startxref's value
                                            ...
                                            } else $this->Error("startxref value expected, read $CurLine");
                                            break;
                                        default://%%EOF
                                    }
                                    $trailer_table--;
        ...
        function parseFDFContent()
        {
            ...
            if (preg_match_all("/(T|V)\s*\(([^\)]+)\)\s*\/(T|V)\s*\(([^\)]+)\)/", $content, $matches, PREG_PATTERN_ORDER)) {
                ...
                for ($f = 0; $f < $fMax; $f++) {
                    ...
                    if ($matches[1][$f] == "V") {
                        $value = $matches[2][$f];
                        if ($matches[3][$f] == "T") $name  = $matches[4][$f];
                        else $this->Error("Field $f ignored , incomplete field declaration, name is expected");
                    } else {
                        if ($matches[1][$f] == "T") {
                            $name  = $matches[2][$f];
                            if ($matches[3][$f] == "V") $value = $matches[4][$f];
                            else $this->Error("Field $f ignored , incomplete field declaration, value is expected");
                        } else $this->Error("Field $f ignored , Invalid field keys ({$matches[0][$f]})");
                    }
                    if ($name != '') {
                        if (array_key_exists($name, $fields)) $this->Error("Field $f ignored , already defined");
                        else {
                            $name          = $this->_unprotectContentValues($name);
                            $value         = $this->_unprotectContentValues($value);
                            if ($this->verbose) $this->dumpContent("FDF field [$name] has its value set to \"$value\"");
                            $fields[$name] = $value;
                        }
                    } else $this->Error("Field $f ignored , no name");
                }
            } else
